...
- `main`、`master`、`develop` などの共有ブランチへ、AIエージェントが直接コミットまたはpushしてはならない。
- 実装、修正、レビュー対応、ドキュメント更新など、変更を伴う作業は必ず作業用ブランチを作成してから行う。
- 作業開始前に現在のブランチを確認し、共有ブランチ上にいる場合は、作業内容に応じた `feature/...`、`fix/...`、`docs/...` などのブランチを切る。
- ユーザーが「pushして」「pushまで対応」「pushしていないならそこまで」などと依頼した場合でも、共有ブランチへの直接push許可とは解釈しない。必ず作業ブランチへpushする。
- 共有ブランチへ直接pushが必要に見える場合は、実行前に対象ブランチ名、push内容、理由を明示してユーザーの明示許可を取る。
- 誤って共有ブランチへpushした場合は、履歴改変を行わず、まず作業ブランチへ当該コミットを退避し、その後ユーザー確認のうえ共有ブランチにrevertコミットを積んで戻す。
- `git reset --hard`、`git push --force`、`git push --force-with-lease` など履歴や作業内容を破壊・改変する操作は、ユーザーの明示許可なしに実行しない。
