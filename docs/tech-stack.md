...
- 元データはiPhone写真ライブラリおよびiCloud写真に残し、アプリは元データを変更・削除しない。
- 実装では写真ライブラリの削除・変更系APIを使用しない。アプリ内の削除はFirestore参照とFirebase Cloud Storage上の派生ファイルだけを対象にする。
- 結果発表の公開条件は、クライアント表示制御だけでなくFirestore Security Rulesでも保護する。
- リセット時にStorage上の不要な派生メディアを削除する要件があるため、参照元、所有スコープ、削除対象を設計時に明確化する。
- ローカルキャッシュは、実装段階で必要性を見てファイルキャッシュまたはSwiftDataを検討する。

## メディア処理方針

- アップロード前に写真・動画を表示・再生用途へ軽量化する。
- 通常の写真は表示用サイズへ縮小し、過度なStorage容量と転送量を避ける。
- 動画はアプリ内再生に支障がない範囲で圧縮版を保存する。
- Live Photosは通常写真として潰さず、静止画部分とモーション部分を対応付けて、アプリ内でLive Photosとして再生できる形式で保存する。
...
- 重点シナリオ:
  ...
  - Storage上の不要な派生メディア削除が、必要なメディアや端末/iCloud写真ライブラリ上の元データを巻き込まない。
...
- Dynamic Type、十分なコントラスト、44pt以上のタップ領域を意識する。
